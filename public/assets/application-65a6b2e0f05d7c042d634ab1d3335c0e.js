...
(function() {
  var attachClipboardEvent;

  attachClipboardEvent = function() {
    var copy_sel;
    $("[rel=tooltip]").tooltip();
    copy_sel = $('.copy_to_clipboard_icon');
    copy_sel.off('click');
    copy_sel.on('click', function(event) {
      return event.preventDefault();
    });
    return copy_sel.clipboard({
  ...
  $(function() {
    ...
    makeSearch = function() {
      var search_string, val;
      val = $('.search-field').val();
      if (val.length < 2) {
        return;
      }
      search_string = val.replace(/\s/g, '-');
      return window.location = '/commands/search/' + search_string;
    };
    $('.search-field').focus();
    ...
    $('#create_an_example_button').click(function() {
      var user_id;
      user_id = $('#user_id').val();
      if (user_id === void 0 || user_id === '') {
        return $('#create_an_example').modal();
      } else {
        return window.location = '/commands/new';
      }
    });
    return $('.load-more-commands').on('inview', function(e, visible) {
      if (!visible) {
        return;
      }
      return $.getScript($(this).attr('href')).done(function() {
        return attachClipboardEvent();
      });
    });
  });
